...
public class Graph implements GraphInterface{
	...
	private void setIncomingDegreeRecursively(GraphNode node) {
		...
		GenericListInterface<GraphEdge> outgoing = node.getOutgoingEdges();
		for (GraphEdge edge : outgoing) {
			if (edge.getEndPoint().getDegree() == 0) {
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
		...
		// initialise the degree attribute of each node
		setIncomingDegree();
		// adds start to temp
		temp.enqueue(start);
		// while temp not empty
		while (!temp.isEmpty()) {
			// dequeue from temp and add to result
			GraphNode current = temp.dequeue();
			result.enqueue(current);
			// decrement degree children by 1
			// any child who's degree = 0 is added to temp
			for (GraphEdge edge : current.getOutgoingEdges()) {
	...
	private void computeEarliestCompletionTime( ) {
		...
		while (!sortedNodes.isEmpty()) {
			// calculate completion times for all incoming edges
			GraphNode node = sortedNodes.dequeue();
			List<Integer> completions = new ArrayList<>();
			for (GraphEdge edge : node.getIncomingEdges()) {
				completions.add(edge.getTaskDuration() +
								edge.getStartPoint().EarliestCompletionTime());
			}
			// go for the maximum
			Integer maxTime = completions
							.stream()
							.mapToInt(v -> v)
							.max().orElse(0);
